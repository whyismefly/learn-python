...
"""
# stock_hist_data.to_sql(stockid+"_hist_data"+str(today),engine,if_exists='append',dtype={'date':sqlalchemy.types.VARCHAR(length=20)})
# #自己设定的VARCHAR长度，可修改，建议使用下行这种形式
stock_hist_data.to_sql(stockid+"_hist_data",engine,if_exists='append',dtype={'date':sqlalchemy.types.VARCHAR(stock_hist_data.index.get_level_values("date").str.len().max())})
